...
L = csmu.Logger('csmu.dftransform');
...
tform = csmu.df2tform(rotation, translation, doReverse);
   ...
   L.debug('Performing trivial transform')
    ...
    for i = 1:cacheLength
        isSame = [all(tformCache(i).tform.T(:) == tform.T(:)), ...
            csmu.refeq(tformCache(i).RApassed, RApassed), ...
            csmu.refeq(tformCache(i).RBpassed, RBpassed)];
        if all(isSame)
            doLoad = i;
            break
        end
    ...
    if isempty(doLoad)
        if doSave
            [B, RB, Aidx, Bfilt] = csmu.affinewarp(A, RA, tform, ...
                warpArgs{:});
            ...
            [B, RB] = csmu.affinewarp(A, RA, tform, warpArgs{:});
        end
    else
...
L = csmu.Logger('csmu.dftransform>parseInputs');
p = inputParser;
