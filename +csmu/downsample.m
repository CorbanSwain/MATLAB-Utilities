...
p = inputParser;
p.addOptional('dim', DEFAULT_DIM, @(x) isvector(x) && all(csmu.isint(x)) ...
   && all(x > 0));
p.parse(varargin{:});
dim = p.Results.dim;
...
sz = size(I);
downsampSel = csmu.cellmap(@(f, i) 1:f:sz(i), num2cell(allfactors), ...
   num2cell(1:nd));
Idown = I(downsampSel{:});
end

function unittest
L = csmu.Logger('csmu.downsample>unittest');
logtest = @(name) L.info('Beginning %s test ...', name);
logpass = @() L.info('\tpassed.');

logtest('unity downsample');
L.assert(all(all(csmu.downsample(magic(5), 1) == magic(5))));
logpass();

logtest('simple 1D 2 factor downsample');
L.assert(all(csmu.downsample(1:10, 2) == (1:2:10))); 
logpass();
...
L.assert(all(all(csmu.downsample(Iin, [2, 4]) == Iout)));
L.assert(all(all(csmu.downsample(Iin, [4, 2], [2 1]) == Iout)));
logpass();
...
t = tic;
L.assert(all(all(all(csmu.downsample(Iin, 3, 1:3) == Iout))));
