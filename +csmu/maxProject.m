...
if isempty(colorwt)
   Vgray = csmu.rgb2gray3d(V);
else
   Vgray = csmu.rgb2gray3d(V, colorwt);
end
...
fun = @(a, b, idx) csmu.cell2csl(fun(a, b, idx));
end

function unittest
L = csmu.Logger('csmu.maxProject>unittest');
printtest = @(s) L.info('\t%s ...', s);
printpass = @() L.info('\t\tpassed.');
L.info('Unit tests for csmu.maxProject:\n');

printtest('Simple Test');
V = zeros(3, 3, 3);
V(1, 1, 3) = 1;
mip = csmu.maxProject(V);
...
V(1, 1, 2, 2) = 0.9;
colorwt = [0.5 1 1];
mip = csmu.maxProject(V, 'ColorWeight', colorwt);
assert(mip(1, 1, 1) == 0);
