...
extensionPath = fullfile(csmu.extensionsDir, 'non_rigid_register_4d_djk_ygy');

functionFolders = {'functions', 'functions_affine', 'functions_nonrigid'};
functionFolders = csmu.cellmap(@(f) fullfile(extensionPath, f), ...
   functionFolders);
cleanup.removeFromPath = onCleanup(@() rmpath(functionFolders{:}));

function val = qgf(fieldName, defaultVal)
   [val, regOptions] = csmu.queryGetField(regOptions, fieldName, defaultVal);
end
qgf('Similarity', 'sd');
