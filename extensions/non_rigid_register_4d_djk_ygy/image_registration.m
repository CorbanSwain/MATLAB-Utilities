...
[O_trans,Spacing,MaxItt]=Make_Initial_Grid(O_trans,Spacing,Options,squeeze(Imoving(:,:,:,1)) ,M,IS3D);
...
if ( nargout<5 )
    for kk=1:size(Imoving,4),
        [Ireg(:,:,:,kk)]=bspline_transform(O_trans,squeeze(Imoving(:,:,:,kk)),Spacing,3);
    end
else
    for kk=1:size(Imoving,4),
        [Ireg(:,:,:,kk),B]=bspline_transform(O_trans,squeeze(Imoving(:,:,:,kk)),Spacing,3);
    end
...
if(sum(size(Istatic)-size(Imoving))~=0)
    % Resize the moving image to fit the static image
    if(IS3D)
        Imoving=imresize4d(Imoving,[],size( squeeze(Istatic(:,:,:,1)) ),'cubic');
        if(~isempty(MASKmoving))
            MASKmoving=imresize3d(MASKmoving,[],size(squeeze(Istatic(:,:,:,1))),'cubic');
        end
    else
...
if(IS3D)
    for kk=1:size(Istatic,4),
       ISmoving(:,:,:,kk) =  imgaussian( squeeze(Imoving(:,:,:,kk)) ,2.5,[10 10 10]);
%        ISstatic(:,:,:,kk) =  imgaussian( squeeze(Istatic(:,:,:,kk)) ,2.5,[10 10 10]);
    end
    ISstatic = Istatic;
else
...
for refine_itt=1:3
    if(refine_itt==1)
        if(IS3D)
            clear ITmoving; clear ITstatic;
            for kk=1:size(Istatic,4),
                ITmoving(:,:,:,kk) =  imresize3d(squeeze(ISmoving(:,:,:,kk)),0.25);
                ITstatic(:,:,:,kk) =  imresize3d(squeeze(ISstatic(:,:,:,kk)),0.25);
            end
        else
    ...
    elseif(refine_itt==2)
        ...
        if(IS3D)
            clear ITmoving; clear ITstatic;
            for kk=1:size(Istatic,4),
                ITmoving(:,:,:,kk) =  imresize3d(squeeze(ISmoving(:,:,:,kk)),0.5);
                ITstatic(:,:,:,kk) =  imresize3d(squeeze(ISstatic(:,:,:,kk)),0.5);
            end
        else
    ...
    if(IS3D)
        optim=struct('GradObj','on','GoalsExactAchieve',1,'Display','off','StoreN',10,'HessUpdate','lbfgs','MaxIter',300,'MaxFunEvals',1000,'TolFun',1e-7,'DiffMinChange',1e-5);
    else
    ...
    if(IS3D)
        ...
        [s_Trans, s_Rotation, s_Scale, s_Shear] = affine_parameter_scaling(size(squeeze(ITmoving(:,:,:,1))));
        if(strcmpi(Options.Registration(1),'R'))
