...
if((size(I1,3)>3)&&(isempty(O_trans))&&(isempty(MaskI1))&&(strcmp(type,'sd')))
    ...
    if(isa(I1,'double'))
        if(nargout>1)
            ...
            for kk=1:size(I1,4),
                [eAll(:,kk),mgradAll(:,:,kk)]=affine_error_3d_double(double( squeeze(I1(:,:,:,kk)) ),double( squeeze(I2(:,:,:,kk)) ),double(M),double(mode));
            end
            e = mean(eAll);
            mgrad = mean(mgradAll,3);
%             [e,mgrad]=affine_error_3d_double(double(I1),double(I2),double(M),double(mode));
            ...
            for kk=1:size(I1,4),
                [eAll(:,kk)]=affine_error_3d_double(double( squeeze(I1(:,:,:,kk)) ),double( squeeze(I2(:,:,:,kk)) ),double(M),double(mode));
            end
            e = mean(eAll);
%             e=affine_error_3d_double(double(I1),double(I2),double(M),double(mode));
        ...
        if(nargout>1)
            for kk=1:size(I1,4),
                [eAll(:,kk),mgradAll(:,:,kk)]=affine_error_3d_single(single(squeeze(I1(:,:,:,kk))),single(squeeze(I2(:,:,:,kk))),single(M),single(mode));
            end
            e = mean(eAll);
            mgrad = mean(mgradAll,3);
%             [e,mgrad]=affine_error_3d_single(single(I1),single(I2),single(M),single(mode));
            ...
            for kk=1:size(I1,4),
                [eAll(:,kk)]=affine_error_3d_single(single(squeeze(I1(:,:,:,kk))),single(squeeze(I2(:,:,:,kk))),single(M),single(mode));
            end
            e = mean(eAll);
        end
    end
...
if(isempty(O_trans)),
    ...
    I3=affine_transform4D(I1,M,mode);
...
% registration error calculation.
% e = image_difference(I3,I2,type,Mask);
e = image_difference4D(I3,I2,type,Mask);
