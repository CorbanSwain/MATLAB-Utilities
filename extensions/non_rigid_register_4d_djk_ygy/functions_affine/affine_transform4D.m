function Iout=affine_transform4D(Iin,M,mode)
...
if(size(Iin,3)<4)
    ...
    if(isa(Iin,'double'))
        for kk=1:size(Iin,4),
            Iout(:,:,:,kk) = affine_transform_3d_double(double(squeeze(Iin(:,:,:,kk))),double(M),double(mode));
        end
    else
        for kk=1:size(Iin,4),
            Iout(:,:,:,kk) = affine_transform_3d_single(single(squeeze(Iin(:,:,:,kk))),single(M),single(mode));
        end
    end
end
